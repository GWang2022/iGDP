...
`cp $r1 $outdir/$outfile/read1.fq;
 ...
 cd $outdir/$outfile/;
 cat ../*telo_reads.recall.contigs | sort > telo_reads.recall.contigs
 non_Ciliophora_hit_contigs.pl ../*/*mmseqs.out.tophit | sort | comm -23 - telo_reads.recall.contigs > non_Ciliophora.recall.contigs
 get_contig_seqs.pl raw.assembly.fa non_Ciliophora.recall.contigs > non_Ciliophora.recall.contigs.fa
 ...
 cat telo_reads.recall.contigs classfified.contigs non_Ciliophora.recall.contigs | sort | uniq |comm -23 all.contigs - > unclassfied.contigs
 N50_longer_contigs.pl non_Ciliophora.recall.contigs.fa | grep '>' | sed 's/>//' | sort > N50_longer.contigs
 ...
 metabat_cluster.pl N50_longer.contigs N50_longer_anclassfied.cluster | sort | comm -23 - telo_reads.recall.contigs > $outfile.non_Ciliophora.clustering.contigs;
 cat non_Ciliophora.recall.contigs $outfile.non_Ciliophora.clustering.contigs | sort | uniq > all.non_Ciliophora.contigs;
 comm -23 all.contigs all.non_Ciliophora.contigs | get_contig_seqs.pl raw.assembly.fa - > final_genome.negative.fa
 cp final_genome.negative.fa ../;
